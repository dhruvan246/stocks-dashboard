...
function profitAt(sym, dateInt, basis) {
  ...
  for (const [npIdx, annIdx] of tries) {
    ...
    // YoY% only when the CURRENT quarter is an actual profit (c > 0): a loss→profit turnaround reads
    // positive, but a shrinking loss (−6.5→−2.4) is NOT growth, so yoy is null and it won't qualify —
    // matches Trendlyne/StockView, which exclude loss-making companies from profit-growth screens.
    return { cur: c, base: b, yoy: (c > 0 && b !== 0) ? (c - b) / Math.abs(b) * 100 : null };
  }
  return null;
}
