...
function membersAsOf(name, dstr) {
  ...
  // FUND_ALIAS fold — CONDITIONAL: rosters (esp. fno_history.json) carry the name that TRADED THEN,
  // and renamed companies' price series were stitched under the CURRENT ticker (INFOSYSTCH->INFY,
  // TATAMOTORS->TMPV, HEROHONDA->HEROMOTOCO...), so the join `members.has(m.symbol)` missed them:
  // 3,135/32,073 F&O slots (9.9%) silently dropped (measured 2026-09-01). BUT fold ONLY a roster
  // name that has NO series of its own — many old names (AGCNET, GESHIPPING...) still carry their
  // OWN era tape, and blindly redirecting those to the current key would screen the WRONG series for
  // that era. Guarding on `SERIES[s]` recovers exactly the stitched-away names (N500 rosters are
  // already current-keyed → 0 folds; F&O → 3,135). (Sync: stock-backtest.html)
  return snap ? new Set(snap.symbols.map(s => (SERIES[s] ? s : (FUND_ALIAS[s] || s)))) : new Set();
}
...
// profit data. Rosters (index + fno_history.json) still carry the name that TRADED THEN, but the
// price series were stitched under the CURRENT name — so membersAsOf now folds through this map too
// (2026-09-01). fundamentals are keyed by the CURRENT name; without this, every renamed stock
// (TATAMOTORS→TMPV, PVR→PVRINOX,
...
function profitMetrics(sym, dateInt, basis) {
  ...
  for (const [ni, ai] of tries) {
    ...
    // Quarter-ends are quantized to Mar/Jun/Sep/Dec, so monthIdx steps by exactly 3 per quarter.
    // accel and streak below step by CALENDAR quarter (monthIdx-3), NOT by array index: arr[ci-1]
    // is the previous ROW, which silently skips a quarter our data never captured (the same class
    // the TTM guard above closes; measured 2026-09-01, accel 1,516 cells/810 syms, streak 495/330).
    ...
    let ttm = null; { let ok = true, last4 = [], prev4 = [];
      ...
      if (ok) { for (let k = 0; k < 3; k++) { if (monthIdx(arr[ci - k][0]) - monthIdx(arr[ci - k - 1][0]) !== 3) { ok = false; break; } } }
