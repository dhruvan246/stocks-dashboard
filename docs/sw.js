...
const CACHE = 'sw-shell-v105';  // v105: strategy-backtest.html — config chips + sw-stat record row in the header, not-found state becomes a strategy-picker landing (v104: stock.html no-symbol landing; v103: UI-audit polish; v102: backtest-history overhaul) (v100: TTM contiguity check strengthened to pairwise gaps — a duplicate qe row (APOLLOTYRE/CARBORUNIV) slipped past the endpoint-only check (ENGINE_VER e6->e7) (v99: profitMetrics TTM requires 4 calendar-consecutive quarters, both engine twins (ENGINE_VER e5->e6) ...) )
const SHELL = [
