...
"""
import os, sys, re, json, gzip, html, datetime, argparse, urllib.request
...
def parse_bag(pdf_bytes, doc_fy=None):
    ...
    if not doc_fy:
        m = re.search(r"BUDGET AT A GLANCE\s+(\d{4})-(\d{4})", full, re.I) or re.search(r"Budget at a Glance\s+(\d{4})-(\d{4})", full)
        if m:
    ...
    for p in d:
        ...
        for l in ls:
            if j and re.match(r"^\d{1,2}\.\s*On (Capital|Revenue)$", j[-1]) and re.match(r"^Account", l):
                ...
                j.append(l)
        ls = j
        if any("Total Expenditure" in l for l in ls) and any(re.search(r"On Capital Account|Capital Expenditure", l) for l in ls) \
           and any("Actuals" in l for l in ls[:80]):
            page = ls
            break
    if page is None:
        raise RuntimeError("no Budget-at-a-Glance table page found in %s" % doc_fy)
        ...
        for pat in pats:
            ...
            if hit is not None:
                ...
                if len(vals) == ncol:
                    ...
                    break
    gdp = {}
    for m in re.finditer(r"GDP for (?:BE|Budget FY|FY)\s*(\d{4})-(?:20)?\d{2}[^.]{0,40}?(?:projected|estimated) at\s*[`₹]?\s*([\d,]{6,})\s*crore", full):
        gdp[fy_label(int(m.group(1)))] = int(m.group(2).replace(",", ""))
    for m in re.finditer(r"(?:Advance|Provisional) Estimates (?:for|of) FY\s*(\d{4})-\d{2}\s*(?:of|at)\s*[₹`]?\s*([\d,]{6,})", full):
...
def load_ledger():
    try:
        ...
        return {"_readme": "Budget-at-a-Glance rows per document (crore). cols = column meaning per value; "
...
def seed_budget(ledger, urls):
    for fy, url in urls.items():
        try:
            ...
            print("  budget %s: FAILED %s" % (fy, str(e)[:100]))
            continue
        rec["url"] = url
        ...
        if old and old.get("rows") == rec["rows"] and old.get("gdp") == rec["gdp"]:
            print("  budget %s: unchanged" % rec["doc"])
        else:
            ledger["docs"][rec["doc"]] = rec
            print("  budget %s: %s cols=%s capex=%s te=%s eff=%s gdp=%s" % (rec["doc"], rec["ncol"], [c[0][:2] + c[1][2:4] for c in rec["cols"]],
                  rec["rows"].get("capex"), rec["rows"].get("te"), rec["rows"].get("eff"), rec["gdp"]))
    return ledger
            ...
            for key in ("capex", "te", "eff", "fd", "rev"):
                ...
                if kind == "act_prov":
                    if key + "_act" not in row:
                        row[key + "_act"] = vals[i]
                        row["act_src"] = "BaG %s (provisional)" % rec["doc"]
                elif kind == "act":
                    row[key + "_act"] = vals[i]
                    row["act_src"] = "BaG %s" % rec["doc"]
                else:
                    row[key + "_" + kind] = vals[i]
                    row[kind + "_src"] = "BaG %s" % rec["doc"]
    ...
    for m in cga_monthly:
        if m["ym"].endswith("-03") and m.get("prov"):
            ...
            if "capex_act" not in row:
                ...
                row["prov_src"] = "CGA provisional accounts, March %s" % m["ym"][:4]
    return [series[k] for k in sorted(series)]


# ----------------------------------------------------------------------------- B. CGA monthly
CGA_LABELS = {"Revenue Receipts": "rr", "Tax Revenue (Net)": "tax", "Non-Tax Revenue": "ntr", "Non-Debt Capital Receipts": "ndcr",
              "Total Receipts (1+4)": "tr", "Revenue Expenditure": "re", "of which Interest Payments": "int",
              "Capital Expenditure": "ce", "of which Loans disbursed": "loans", "Total Expenditure (8+10)": "te",
              "Fiscal Deficit (12-7)": "fd", "Revenue Deficit (8-1)": "rd", "Primary Deficit (13-9)": "pd"}
        ...
        if ym not in have or ym in refetch:
            try:
                ...
                if ym in have and have[ym] != rec:
                    print("  CGA %s: revised" % ym)
                elif ym not in have:
                    print("  CGA %s: new (%s %s ce=%.0f of %.0f)" % (ym, rec["est"], rec["fy"], rec["ce"], rec["ce_be"]))
                have[ym] = rec
                fetched += 1
            except urllib.error.HTTPError as e:
                if e.code != 404:
                    print("  CGA %s: HTTP %s" % (ym, e.code))
            except Exception as e:
                print("  CGA %s: %s" % (ym, str(e)[:80]))
        m += 1
        if m > 12:
            m, y = 1, y + 1
...
def load_filings():
    ...
    try:
        ...
        s = snaps[-1]
        n500 = set(x for x in s["symbols"] if not str(x).upper().startswith("DUMMY"))
        n500_date = s["effectiveDate"]
    except Exception as e:
        print("  (Nifty 500 snapshot unavailable: %s)" % str(e)[:60])
...
def members_asof(IH, idx, asof):
    ...
    if not best:
        return None, set()
    s = best[-1]
    return s["effectiveDate"], set(x for x in s["symbols"] if not str(x).upper().startswith("DUMMY"))


GROUPS = {   # listed companies of a promoter group, for the reality-check table (hand list, 2026-09-07)
    "Adani": ["ADANIENT", "ADANIPORTS", "ADANIGREEN", "ADANIPOWER", "ADANIENSOL", "ATGL", "AWL", "ACC", "AMBUJACEM", "NDTV", "SANGHIIND"],
}
...
def build_filings(FYS=(2021, 2022, 2023, 2024, 2025, 2026)):
    ...
    macro = lambda s: (cls.get(s + ".NS") or cls.get(s) or {}).get("macro") or "Unclassified"
    ...
    def universe_series(syms):
        out = {"n": [], "capex": [], "cfo": [], "cfo_n": [], "gt1000": [],
               "panel_n": [], "panel_cur": [], "panel_prev": []}
        for fy in FYS:
            ...
            for s in syms:
                ...
                if a:
                    ...
                    pc += a["capex"]
                    pp += p["capex"]
            out["n"].append(n); out["capex"].append(round(cap)); out["cfo"].append(round(cfo)); out["cfo_n"].append(cfo_n)
            out["gt1000"].append(gt); out["panel_n"].append(pn); out["panel_cur"].append(round(pc)); out["panel_prev"].append(round(pp))
        return out

    def fixed_panel(syms, fys):
        members = [s for s in syms if all(best(s, fy)[0] for fy in fys)]
        return {"fys": list(fys), "n": len(members),
    ...
    for y in range(FYS[-1] - 3, FYS[-1] + 1):
        ...
        for s in nonfin:
            for b in ("c", "s"):
                ...
                if a.get("capex") is not None and p.get("capex") is not None and 85 <= (a.get("cf_d") or 0) <= 230 and 85 <= (p.get("cf_d") or 0) <= 230:
                    n += 1; cur += a["capex"]; prev += p["capex"]
                    break
        if n:
    ...
    for s in nonfin:
        ...
        top.append({"sym": s, "name": names.get(s, s), "macro": macro(s), "basis": b, "capex": round(x["capex"]),
                    "prev": round(p["capex"]) if p else None, "cfo": round(x["cfo"]) if x.get("cfo") is not None else None,
                    "n500": s in n500})
    ...
    for gname, syms in GROUPS.items():
        ...
        groups[gname] = {"syms": have, "capex": {str(fy): round(sum((best(s, fy)[0] or {}).get("capex") or 0 for s in have)) for fy in FYS},
...
def load_gdp():
    ...
    try:
        ...
        print("  (macro.json gdpn unavailable: %s)" % str(e)[:60])
    return out
...
def main():
    ...
    ap.add_argument("--seed-budget", action="store_true", help="parse the whole Budget-at-a-Glance archive into the ledger")
    ...
    try:
        ...
        print("  (official ledger unavailable: %s)" % str(e)[:60])
    ...
    print("Wrote %s: %d annual rows, %d months, filings %s; %s" % (OUT, len(annual), len(monthly), "yes" if filings else "no", "; ".join(log)))
