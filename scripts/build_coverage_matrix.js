...
/* readGz: parse a gzipped-JSON file whose UNCOMPRESSED form may exceed V8's max string length
 * (0x1fffffe8 ≈ 537 M chars) — the live sf bin crossed that line ~2026-09-16 and a one-shot
 * JSON.parse(gunzipSync(...)) now throws ERR_STRING_TOO_LONG. The split-parser (fast native path,
 * falling back to a structural member-by-member split only when a slice would blow the cap) was
 * extracted verbatim to scripts/_read_big_gz.js and is now shared with build_first_bar_map.js /
 * grid_search.js / grid_search_full.js, so the four live-bin readers cannot drift. Memory:
 * project-stocks-live-bin-exceeds-v8-string-cap. */
