...
/* Read a gzipped JSON payload whose UNCOMPRESSED form may exceed V8's maximum string
 * length (0x1fffffe8 ≈ 537 M chars). The live sf bin crossed that line ~2026-09-16
 * (uncompressed ~540 MB) and `JSON.parse(gunzipSync(buf))` began dying with
 * ERR_STRING_TOO_LONG at Buffer.toString — the whole gunzipped Buffer can never become
 * one JS string, which broke this nightly bake three nights running. (Node Buffers hold
 * ~2 GB, far past the string cap, so gunzip itself is fine — only the string is capped.)
 *
 * FAST PATH: for any file under the cap this is exactly the old one-shot parse.
 * FALLBACK: when the slice is too long to stringify, split it one container level at a
 * time and recurse — so a huge top-level object is parsed member-by-member, and the huge
 * `data` map (once it too crosses the cap) is parsed one ~7 KB symbol record at a time.
 * No single string ever approaches the cap. The scan is purely structural (string/escape
 * + {}/[] depth aware) and makes NO assumption about key order, whitespace, or which
 * member is the big one — it only ever splits as deep as the cap forces it to. */
function readGz(p) {
