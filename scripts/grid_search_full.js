...
(function () {
  ...
  // Shared big-gzip reader — see scripts/_read_big_gz.js (live sf bin is past V8's string cap,
  // so a one-shot JSON.parse(gunzipSync(...)) throws ERR_STRING_TOO_LONG).
  ...
  //    fitting strategies on it silently uses ~10-week-old prices AND a different universe. Load the
  //    LIVE release asset (cached, validated, re-downloaded when it falls behind docs/sf_meta.json),
  //    ABORT rather than run stale. SF_BIN=<path> overrides. Mirrors build_coverage_matrix.js. ──
