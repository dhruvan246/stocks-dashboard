...
## M. HINDUNILVR PAT VALUES — the last quantmac residue, CLOSED (2026-08-23)

§K left this open: HINDUNILVR stayed picked in Jan-2009 because our stored PAT VALUES disagreed
with the era's press figures. Audited against BSE's own announcement text (the `MORE` field
carries the filed numbers verbatim), sweeping 21 quarters 2005-2011 — **exactly 3 mismatched, all
inside HUL's December→March fiscal-year transition**, everything else matching to the paisa:

| quarter | stored | BSE filing | evidence (TWO independent documents each) |
|---|---|---|---|
| 2007-09-30 | 409.30 | **408.06** | own filing 2007-10-31 12:48 + the Sep-2008 filing's comparative |
| 2007-12-31 | 473.79 | **631.44** | own filing 2008-02-13 12:42 ("Q4 & FY 07" — Dec-2007 was the last quarter of the December year-end) + the Dec-2008 filing's comparative |
| 2008-12-31 | 632.24 | **615.74** | own filing 2009-01-27 08:44 + the Dec-2009 filing's comparative |

The second document matters: it separates an ERROR from a RESTATEMENT. A later filing that still
quotes 6157.40 for Dec-2008 proves the company never restated it to 632.24 — our number is simply
wrong. (Note 632.24 sits a whisker from the PRIOR year's 631.44, the signature of a
comparative-column mis-read.)

**Route:** new REVIEWED ledger `scripts/fund_cell_fix.json` + `scripts/apply_fund_cell_fix.py`
(dry-run default, guarded on `was`, idempotent, refuses to force a cell someone else has moved).
This class had no ledger — scale_fix.json is for power-of-ten parse errors and does not fit.
Measured before writing: pre-2016 sf_fundamentals cells are NOT regenerated by the nightly
(HINDUNILVR 20071231 held 473.79 unchanged across 40 commits), so the write persists; wiring the
ledger into build_fundamentals.py remains the follow-up for any era that IS rebuilt.

**Result:** YoY for the Jan-2009 screen goes **+33.4% → −2.5%**, matching quantmac and Business
Standard exactly, and the >25% filter it used to pass now correctly fails. A/B: **HINDUNILVR
Jan-2009 is gone from the basket**, 3 baskets change (2009-01, 2009-02, 2019-05), CAGR 52.89 →
