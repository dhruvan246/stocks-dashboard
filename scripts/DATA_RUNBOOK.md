...
- **Schedule: cron `0 18 * * *` UTC = 23:30 IST, ONCE a day.** It ran `0 8,11,15,18 * * *` UTC
  ...
  while the other three landed nothing).
  23:30 is the correct SINGLE slot for two measured reasons. (a) It is LAST, so every free reader has
  already had its turn — BSE 12:10 + 20:10, refresh-fundamentals hourly 09:30–19:30 plus the 21:15
  nightly, refresh-announcements 21:30, refresh-results-hourly through 00:30 — honoring the standing
  contract *"other fetches first; vision only fills what they missed"*. (b) Filings peak 16:00–19:00
  IST and **99.5% of a day's arrive by 22:00** (measured across the 4,404-row results_feed), so a
  23:30 run sees the day COMPLETE — where 16:30 would see 33% and 13:30 only 17%.
...
- **Landing path — direct push to main is 403-blocked for cloud sessions.** The routine pushes a
  ...
  no next slot for 24 h.** The prompt was updated to match — 3 recovery cycles instead of 2, and a
...
| All Picks rewind showed **"not started"** (blank return AND blank holdings) for any strategy whose saved backtest window began after the rewind date — `lastRebalanceDate()` read `cfg.start` as a deployment date when it is only the From box that happened to be on the backtest form at save time ([[project-stocks-cfg-start-is-not-a-start-date]], 2026-08-13) | **new** — before GATING anything on a field, establish what the field *means*, and prefer the codebase's own answer over the name: `bt-identity.js` explicitly excludes `start`/`end` from `identityKey`, i.e. the repo already declared it metadata, not state. Two cheap tests that both caught this in minutes: (a) **vary the field and see if the output moves** — start 2004/2020/2023 all anchored to the same month-end, so its ONLY effect was the null; a field that changes nothing except whether you refuse to render is not load-bearing, it's a gate you invented; (b) **diff against the other page computing the same quantity** — `strategy-backtest.html` `monthEndBefore()` never had the gate, so the two pages disagreed about one strategy. Also: when an empty result gets a user-facing label, the label is a CLAIM — "in cash" / "no stock qualified" assert a screen ran; if it couldn't run, say so ("no data"), and put the wording in ONE helper both views call |
| Nightly coverage matrix died on a bare `Z_DATA_ERROR: incorrect header check` **while the release asset was perfectly healthy** — `resolveBin()` ran `curl -sL -o` with no `--fail`, so a transient HTTP error exited 0 and wrote the error BODY where a 193 MB gzip was expected (2026-08-12, fixed 2a9ab7d1) | **new** — **`curl` exit 0 does not mean you got the file.** Any `curl … -o` that something later parses needs (a) `-f` so a >=400 exits non-zero (`-L` does NOT fix it — it saves the error page instead) and `-S` to keep the message under `-s`; (b) a **content** check before parsing — gzip magic `1f 8b`, a size floor, or a record-count floor. A size-only gate is not enough: a >50 MB non-gzip file passed this builder's old `size < 5e7` check and would have been cached and reused forever; (c) **delete the reject** — a bad file left on disk is what the next run's cache check mistakes for a good one — and log why, echoing the first ~300 bytes so the next failure diagnoses itself. Sweep siblings by BEHAVIOUR (`grep` every `curl … -o`, then read what each does with the file): `update_sf_data.py` and `backup-backtest-history.yml` already validated; this builder was the lone straggler. Second instance of the class — BSE's 162-byte 302 stub (§0) was the first |
| **A CORRECT fill stopped the whole fundamentals pipeline — twice in one evening.** Two fill passes (17:00 IST `561ddd75`; 20:14/20:20 IST `f24388563`+`6ae9d2a81`) wrote consolidated cells read properly from the filings — but those same cells were flagged `held` in `mc_history_fills.json`/`mc_pat_fills.json`, and a held cell asserts ABSENCE (§56b). The blocking `verify_fills_live.py` step in `refresh-fundamentals.yml` exited 1 on RESURRECTED, aborting **before** the commit step, so nothing published for hours while `auto-rerun.yml` mailed 6 notices per failed run (2026-08-16, fixed d4786dfef + 5926863ac) | **new** — **a settle is TWO writes, and the second one is the one that keeps CI green.** Writing the value is only half; the other half is lifting or annotating the hold in **every** ledger that claims that cell, **in the same commit**. So before pushing any fill: (a) grep all of `scripts/` for each cell key in all three shapes (`SYM|QE`, `SYM|QE|con`, `SYM|QE|revC`) *and* the nested `{SYM: {QE: …}}` stores — a cell is routinely claimed by several campaigns with contradictory verdicts, and whichever applier runs last decides the store; (b) **run `python3 -X utf8 scripts/verify_fills_live.py` before the push** — it is the identical check CI blocks on, it takes under a second, and it would have caught both waves; (c) **register any NEW ledger in that file at creation** — `conpat_filing_fills.json` shipped unguarded, and an unregistered ledger reports a serene `MISSING 0` while checking nothing. Note the count does NOT prove a lift (a held entry already increments `checked`); prove it with a negative control on sandbox COPIES — null the cell, expect exit 1 naming both ledgers. **★ IT HAPPENED A THIRD TIME (JSWDULUX 2018-09/12, 2026-08-17), so (b) is now ENFORCED rather than remembered, at both ends:** `scripts/githooks/pre-commit` (install once per clone with `bash scripts/install_git_hooks.sh`) refuses any commit that stages a payload or ledger while a held cell is resurrected, naming the cell and the fix — all three incidents were authored on this machine, so that is where the cheap catch is; and `scripts/settle_stale_holds.py --apply` runs in `refresh-fundamentals.yml` just before the blocking step to self-heal the mechanical case, lifting a hold ONLY when a filing-sourced ledger asserts that same cell at that same value (the route the holds' own text names). A resurrection with no filing behind it still fails loudly — that is the case the guard exists for, and auto-lifting on "the value is live" would delete the guard, since 6 held flags AND 7 of 28 retractions have been measured wrong |
| **`<circle cx="NaN">` on every load of 666 stock pages — and the same sentinel was leaking a REWIND LOOK-AHEAD.** `ann=0` ("announce date unknown", §91) was gated with `!= null` in 5 places in `docs/stock.html`, the ONE consumer never brought in line when the engines were fixed (2026-08-12). `annOf` → `ymd2iso(0)` = `"0--"` → `dayOff` = NaN → the marker loop's range filter `e.off<x0||e.off>x1` **cannot reject a NaN** (both comparisons false), so it reached the SVG. Measured before the fix: 1,442 cells / 666 stocks; `"0--"` printed as the Announced date on 200; `Math.min(0, realDate)` destroyed a REAL date on the other basis on 119 rows (BCPL showed `0--` ×5 though con carried 2025-08-14 …); and HUHTAMAKI rewound to 2012-01-01 listed 2012-06 … 2013-06 — 18 months of unfiled results — in the view whose own comment promises "what the market actually knew then" (2026-08-18, fixed c1e1a2693) | **new** — (a) **when a sentinel gets fixed, fix it in every TWIN in the same pass**: `backtest-engine.js` and `stock-backtest.html` both carry a `⚠️ ann must be TRUTHY` comment; `stock.html` reads the same rows from the same file and was never touched, so the class survived 6 days in the most-visited page on the site. `grep -n 'ann\|\[2\]!=null\|\[4\]!=null'` **every** reader of the JSON, not just the one that raised the alarm. (b) **A range filter is not a NaN filter.** Any `if(v<lo||v>hi) continue` guarding a computed SVG/CSS attribute needs `Number.isFinite(v)` FIRST, or a bad value takes the *pass* branch. (c) **Rendering-layer errors have no JS stack and no `console.error` call site** — `read_console_messages` sees them, but patching `setAttribute`/`innerHTML` to trap `NaN` finds nothing if the state has since settled; the fastest route was to compute the offending value from the DATA (`EVENTS.filter(e=>!Number.isFinite(e.off))`) instead of hunting the render. (d) A bug the reporter calls "not data-specific" (2 unrelated symbols) can still be data-dependent — INFY is clean, 666 stocks are not; **count the affected population before scoping the fix**, it is what turned a console-noise ticket into a point-in-time-integrity fix |
...
### 91m. The third twin: `docs/stock.html` (2026-08-18)
§91's fix landed in `backtest-engine.js` and `stock-backtest.html` and stopped there. **`docs/stock.html`
read the same `ann` fields with `!= null` for six more days** — five sites: `annOf()`, `reactionAt()`, the
`kStd`/`kCon` rewind gate, and (through `annOf`) the `withAnn` last-result line and the `annCell` date.
Population measured off `docs/fin/*.json`: **1,442 sentinel cells on 666 of 4,630 stocks**, of which
**200 print `"0--"` in the visible last-12 quarter table** and **119 sit beside a REAL date on the other
basis**, where `Math.min(0, real)` threw the real one away. Fixed with `annReal(v)=v>0?v:null` + `>0` in
the gate (c1e1a2693). Two transferable lessons: the sentinel's damage is **not** limited to the consumer
that reports it (here the visible symptom was a console error; the expensive defect was a rewind
look-ahead of 18 months); and `dayOff(ymd2iso(0))` = `dayOff('0--')` = **NaN silently passes any
`v<lo||v>hi` filter**, so guards on computed SVG/CSS attributes need `Number.isFinite` first.
...
|---|---|---|---|
| 1 | Index membership not point-in-time (SANDESH/PCBL/CRESTANI/FOSECOIND/…) | 24 | **CONFIRMED, OPEN — both sides now measured (2026-08-20 review pass).** Our side: all 24 flagged symbol/date pairs read as members in `indices_history.json["Nifty 500"]`. NSE's side, first written up on quantmac's citation alone — now independently confirmed via 3 primary sources: NSE's own `IndexInclExcl.xls` (2,496 events 1998-2020, saved to `scripts/_staleness_fix/`), the underlying press-release PDFs, and archived official constituent CSVs. All 5 highest-signal claims match to the day; PCBL is excluded 2002-01-17 to 2018-09-28 (16 years), so every 2017 PCBL trade sits inside that window. Root-cause locus is self-documented in our own code: `build_membership_v2.py`'s phantom-floor guard admits it can't see a "continuously-listed but excluded" name. Not a code bug in `membersAsOf`/`lastSnap` — the reconstruction upstream of them. Fix plan: `scripts/PLAN_QUANTMAC_FIXES.md` P3. |
...
| 3 | Stale fundamentals at rebalance (HINDUNILVR/MARICO/HEROMOTOCO/…) | 8 | **CONFIRMED, OPEN, root cause found — now 8/8 verified (2026-08-20 review pass), and the error is BIDIRECTIONAL.** All 8 confirmed against live BSE records, several via the filed PDF itself (document-level proof), matching quantmac to the minute where they gave one. Root cause: our stored ann-dates for the older era are a `quarter-end + 45 days` formulaic fallback (`apply_agg_pat_fills.py`'s documented convention — MEASURED 2026-08-20: **36,027 cells / 2,623 symbols** dataset-wide carry this exact signature, not just 8), not the true filing date. **Correction to the first-pass framing: this is NOT purely "stale."** OMAXE's Mar-2012 quarter shows the placeholder can be *early* too (real filing 2012-05-30, placeholder 2012-05-15) — a look-ahead in the opposite direction, plausibly structural for March/Q4 quarters (era Clause 41 gave audited annual results 60 days vs 45 for other quarters). The fix is a re-dating campaign (both directions), not a one-directional staleness patch. Plan: `scripts/PLAN_QUANTMAC_FIXES.md` P1/P2. |
