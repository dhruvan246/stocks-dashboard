...
# mql5 slug -> series key
MQ = {
    "repo":     "rbi-interest-rate-decision",
    "cpi":      "cpi-yy",
    "iip":      "industrial-production-yy",
    "gdp":      "gdp-yy",
    "m3":       "rbi-m3-money-supply-yy",
    "deposits": "deposit-growth-yy",
    "forex":    "foreign-exchange-reserves",
}
MQ_BASE = "https://www.mql5.com/en/economic-calendar/india/"
...
def fetch_mql5(slug):
    ...
    url = MQ_BASE + slug + "/export"
    req = urllib.request.Request(url, headers={
        "User-Agent": UA, "Accept": "text/csv,text/plain,*/*",
        "Accept-Language": "en-US,en;q=0.9", "Referer": MQ_BASE + slug})
...
def main():
    ...
    for key in ("us10y", "usdinr", "dxy", "crude"):
        if yh.get(key):
            _merge(out, key, yh[key], rnd=3 if key in ("usdinr",) else 2)
    # gold: USD/oz -> Rs/10g using same-day USD/INR (fallback: latest INR)
    if yh.get("goldusd") and yh.get("usdinr"):
        inr = yh["usdinr"]
        last_inr = inr[max(inr)] if inr else None
        gold = {}
        for date, usd in yh["goldusd"].items():
            rate = inr.get(date, last_inr)
            if rate:
                gold[date] = usd * rate * OZ_TO_10G
        if gold:
            _merge(out, "gold", gold, rnd=0)

    # --- mql5 (economy) ---
    for key, slug in MQ.items():
        try:
            _merge(out, key, fetch_mql5(slug), rnd=1)
            ok.append("mq:" + key)
        except Exception as e:
